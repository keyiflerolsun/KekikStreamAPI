...
export default class VideoPlayer {
    ...
    loadVideo(index) {
        ...
        let proxyUrl = `http://127.0.0.1:3311/proxy/video?url=${encodeURIComponent(originalUrl)}`;
        if (referer) {
        ...
        if (selectedVideo.subtitles && selectedVideo.subtitles.length > 0) {
            ...
            selectedVideo.subtitles.forEach(subtitle => {
                try {
                    ...
                    let subtitleProxyUrl = `http://127.0.0.1:3311/proxy/subtitle?url=${encodeURIComponent(subtitle.url)}`;
    ...
    loadHLSVideo(originalUrl, referer, userAgent) {
        ...
        if (Hls.isSupported()) {
            ...
            try {
                    ...
                    xhrSetup: (xhr, url) => {
                        ...
                        if (url.startsWith(window.location.origin) && !url.includes('/proxy/')) {
                            ...
                            if (this.lastLoadedOrigin) {
                                ...
                                let proxyUrl = `http://127.0.0.1:3311/proxy/video?url=${encodeURIComponent(correctUrl)}`;
                        ...
                        try {
                            let proxyUrl = `http://127.0.0.1:3311/proxy/video?url=${encodeURIComponent(url)}`;
                            if (referer) proxyUrl += `&referer=${encodeURIComponent(referer)}`;
                            if (userAgent) proxyUrl += `&user_agent=${encodeURIComponent(userAgent)}`;
                            ...
                            let fallbackUrl = `http://127.0.0.1:3311/proxy/video?url=${encodeURIComponent(url)}`;
                ...
                let manifestProxyUrl = `http://127.0.0.1:3311/proxy/video?url=${encodeURIComponent(originalUrl)}`;
